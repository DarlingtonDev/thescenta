...
const Filter = () => {
  ...
  const [filterState, setFilterState] = useState<string | boolean>(false);
  const searchParams = useSearchParams();
  const router = useRouter();
  function handleFilter(value: string) {
    const paramsUrl = new URLSearchParams(searchParams.toString());
    setFilterState(value.replace(value[0], value[0].toUpperCase()));
    paramsUrl.set("filter", value);
    router.push(`/admin/products?${paramsUrl.toString()}`);
    setFilterOpen(false);
  }
  return (
    <div className="w-full sm:w-auto min-w-[100px] py-1.5 border border-gray-300 flex justify-center items-center relative">
      <button
      ...
      {filterOpen && (
        <ul className="flex flex-col gap-y-4 w-fit absolute bg-white py-1 px-2 text-xs font-inter font-medium top-7 items-center  border border-gray-300">
          <li className="cursor-pointer" onClick={() => handleFilter("active")}>
            Active
          </li>
          <li className="cursor-pointer" onClick={() => handleFilter("drafts")}>
            Drafts
          </li>
          {/* <li className="cursor-pointer" onClick={() => handleFilter("oldest")}>
            Created
          </li> */}
